...
# open NN loss
with open("./my_idea2/NF/training_loss_NF.npy", mode="rb") as file:
    nf_loss = np.load(file)
with open("./my_idea2/NF30/training_loss_NF30.npy", mode="rb") as file:
    nf30_loss = np.load(file)

# open NN preds
with open("./my_idea2/NF/preds_NF.npy", mode="rb") as file:
    nf_preds = np.load(file)
with open("./my_idea2/NF30/preds_NF30.npy", mode="rb") as file:
...
plt.boxplot([nf_true_difference[:, 0], ml_true_difference[:, 0]], labels=["NF", "ML"])
plt.show()

plt.figure()
plt.title("Spatial-range box plots")
plt.ylabel("spatial-range preds - spatial-range")
plt.boxplot([nf_true_difference[:, 1], ml_true_difference[:, 1]], labels=["NF", "ML"])
plt.show()

plt.figure()
plt.title("Log-nugget box plots")
plt.ylabel("log-nugget preds - log-nugget")
plt.boxplot([nf30_true_difference[:, 0], ml30_true_difference[:, 0]],
            labels=["NF30", "ML30"])
plt.show()
...
plt.title("Spatial-range box plots")
plt.ylabel("spatial-range preds - spatial-range")
plt.boxplot([nf30_true_difference[:, 1], ml30_true_difference[:, 1]],
            labels=["NF30", "ML30"])
plt.show()
