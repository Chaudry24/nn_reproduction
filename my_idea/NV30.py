...
# open test data
with open("../npy/test30_semivariogram_my_idea.npy", mode="rb") as file:
...
for i in range(n_epochs):
    ...
    if i % N == 0:
        ...
        semi_variogram_train_30 = np.array(joblib.Parallel(-1)(joblib.delayed(some_file_1.Spatial.compute_semivariogram)
                                                               (spatial_grid,
                                                                observations_train_30[i, :, :, j].reshape(256, -1))
                                                               for i in range(training_parameter_space.shape[0])
                                                               for j in range(30))).reshape(
            training_parameter_space.shape[0], -1)
