...
#    for i in range(30):
#        tmp = tf.expand_dims(observations_train_30[:, :, :, i], 3)
...
# load model
model_NF30 = tf.keras.models.load_model("./NF/NF_model.h5")

preds_NF30 = np.empty((observations_test_30.shape[0], 2))

for j in range(observations_test_30.shape[0]):
    tmp = np.empty((30, 2))
    for i in range(30):
        tmp3 = tf.expand_dims(tf.convert_to_tensor(observations_test_30)[j, :, :, i], 2)
        tmp2 = model_NF30.predict(x=tf.expand_dims(tmp3, 0))
        tmp[i, 0] = tmp2[0][0]
        tmp[i, 1] = tmp2[0][1]
